...
// USING ENTITIES (Entidad--Entidad):
  ...
  // Relation between "juez" and thesauri "sexo" (IS_A)
  .then(function() {
    return Promise.all(docs.entity.map(function(entity){
      if(!entity.metadata.sexo){
        return Promise.resolve()
      ...
      return query(
        "WITH {json} AS data UNWIND data AS sexoId MATCH (dictionary:Dictionary {type: 'Sexo'}), (juez:Entidad) WHERE juez.id='"+entity._id+"' AND sexoId=dictionary.value CREATE (juez)-[:IS_A]->(dictionary) ",
        {json: entity.metadata.sexo}
      );    
    }))
  })
  ...
  // Delete entity.id and document.id (later on we should change it not saving the id from couchdb, only reading it) 
  .then(function() {
    return query(
      "MATCH (d:Documento), (e:Entidad) REMOVE d.id, e.id");    
  })
